...
public class ServerThread implements Runnable {
    ...
    return user1.getID() + "," +
           ...
           user1.getNumberOfWin() + "," +
           user1.getNumberOfDraw() + "," +
           user1.getRank();
}
        ...
        try {
            ...
            while (!isClosed) {
                ...
                if (messageSplit[0].equals("client-verify")) {
                    ...
                    String rawPassword = messageSplit[2];

                    User user1 = userDAO.findByUsername(username); 

                    if (user1 == null) {
                        write("wrong-user," + username);
                    }
                    else if (!PasswordUtils.check(rawPassword, user1.getPassword())) {
                        write("wrong-user," + username);
                    }
                        ...
                        Server.serverThreadBus.boardCast(
                            clientNumber,
                            "chat-server," + user1.getNickname() + " đang online"
                        );

                        Server.admin.addMessage(
                            "[" + user1.getID() + "] " + user1.getNickname() + " đang online"
                        );

                    }
                    else if (!userDAO.checkIsBanned(user1)) {
                        write("dupplicate-login," + username);
                    }
                    else {
                        write("banned-user," + username);
                    }
                }
                ...
                if (messageSplit[0].equals("register")) {
                    ...
                    if (checkdup) {
                        ...
                        String username = messageSplit[1];
                        String rawPassword = messageSplit[2];
                        String nickname = messageSplit[3];
                        String avatar = messageSplit[4];
                        ...
                        Server.serverThreadBus.boardCast(
                            clientNumber,
                            "chat-server," + this.user.getNickname() + " đang online"
                        );
